            ...
            const res = cart.map(product => {
                if (product.id == item.id) {
                    return {
                        ...product,
                        qty: product.qty + 1,
                        totalPrice: product.price * ( product.qty + 1 ) // product.totalPrice + product.price
                    }
                }
                return product
            })
    ...
    return (
        <Fragment>
            <div className='mt-20 gap-2 px-2 md:px-0 flex justify-center'>
                <input className='outline-none p-2.5 w-full md:w-[500px]' type="text" placeholder='Search...' value={query} onChange={({ target }) => setQuery(target.value)}/>
                <select onChange={({ target }) => {
                    setSort(target.value)
                    handleSort(target.value)
                }} defaultValue={sort} className='outline-none'>
            ...
            <div className='mt-5 flex flex-wrap gap-3 px-6 justify-center'>
                {
                    products.map(item => {
                        ...
                        return (
                            <div key={item.id} className='w-52 bg-white pb-2'>
                                <div className='p-2'>
                                ...
                                <div className='px-2 text-center'>
                                    ...
                                    <div className='flex flex-col gap-2 mt-2'>
                                        <button onClick={() => handleAddCart(item)} type="button" className='bg-purple-600 text-white p-1 w-full'>
                                            <i className='fa fa-cart-shopping' /> Add To Cart</button>
                                        <button type="button" className='bg-green-600 text-white p-1 w-full'><i className='fa fa-dollar'/> Buy Now</button>
                                    </div>
                                </div>
                            </div>
                        )
